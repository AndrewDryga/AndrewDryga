...
We had our own caching layer in front of DataBricks but even with that and CDN in front of it some small percentage of requests still went to DataBricks leading to P99 latency being for data-requests being in seconds. I've rebuild caching layer to use DuckDB embedded in Elixir and our data engineer (shotout to Rafael) changed our pyspark codebase to export parquet files to GCS bucket. New data layer now watches for changes in those files, downloads them and atomically swaps tables making to never drop a single request during the update. The result was that we stopped paying for DataBricks native serving and P99 latency for data-requests dropped to <100ms.
