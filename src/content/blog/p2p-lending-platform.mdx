...
export const BucketBar = ({ label, value, barClass, valueClass }) => (
...
export const initialBucketPlan = [
  { label: "Low-risk bucket (target 75%)", value: 10, barClass: "bg-primary", valueClass: "text-primary" },
  { label: "High-risk bucket (target 25%)", value: 10, barClass: "bg-secondary", valueClass: "text-secondary" },
];

export const rebalancedBucketPlan = [
  { label: "Low-risk bucket (remaining demand)", value: 65, barClass: "bg-primary", valueClass: "text-primary" },
  { label: "High-risk bucket (remaining demand)", value: 35, barClass: "bg-secondary", valueClass: "text-secondary" },
];
...
- From day one we should be ready to process hundreds of thousands of loans per day. A large partner was waiting to integrate from day zero.
- Maintenance must be hand-off friendly. Once the build is stable, the team should be able to move on without jeopardising operations.
- Business users need configuration superpowers. Shipping a new product shouldn’t require code for every tweak.
- Hard constraints: 6 months, ≈10 engineers, a couple of analysts, and a small business squad.
- Integrations should be as simple as possible for originators; changes are far more expensive for them than for us.
- This is regulated fintech: every byte must be traceable, audit-friendly, and resilient to rounding or floating-point surprises.
- Loans are just the first asset class; the architecture has to extend to other instruments. _(In this article “loan” and “asset” are interchangeable.)_
...
- Code that is dramatically easier to write, read, and maintain.
- Fault tolerance out of the box.
- True concurrency without gymnastics.
- Stream-first data processing.
- One release artifact promoted across environments.
- No more bespoke server setups or drift between environments.
- Well-structured front-end applications with SSR.
- BPMN specifications from analysts map cleanly to code thanks to pattern matching and lightweight processes.
...
- **Asset Processor: ** communicates with originators, enriches their data, and turns assets into sell offers.
- **Trader: ** a trading bot that places buy offers based on each investor’s portfolio criteria.
- **Matcher: ** pairs sell and buy offers and prepares the data to create investments.
- **Asset Management System (AMS): ** moves money, keeps the ledger coherent, and feeds reports.
- **CRM: ** centralises customer lifecycle management and product configuration.

<SequenceDiagram className="my-6">
  <Participants>
    <Participant name="Originator" position="left" color="accent" />
    <Participant name="Marketplace (Asset Processor)" position="right" color="primary" />
  </Participants>
  <Messages>
    <Message label="Upload BSON batch" />
    <Message label="Stream + diff assets" type="async" direction="left" />
    <Message label="Publish sell offers into queues" />
    <Message label="Ack + delivery reports" type="response" />
  </Messages>
  <Note text="Batches land via multipart uploads; we diff, enrich, and push them through the trading pipeline." />
</SequenceDiagram>
...
- Persist and enrich the incoming data.
...
- Establishing a new connection per asset is expensive, especially for legacy originator systems. **We want batched uploads.**
- Most originators run nightly jobs; batches can therefore be huge.
- Intraday updates must stay possible (e.g., recalling a repaid loan).
- JSON parsing requires loading the entire document into memory; BSON lets us stream.
...
> If you haven’t watched José Valim’s GenStage keynote, do it. The producer / consumer back-pressure model was the missing ingredient for our ingestion pipeline.
...
- **Integration Layer** — receives batches, routes work, and owns the business process.
- **Assessor** — predicts cash flow and classifies assets by risk group.
...
8. **Assessor/Producer-Consumer**: Call the classifier service (same) and enrich the data once more;
9. **Assessor/Producer**: Publish the message to Assessor.Out RabbitMQ queue and acknowledge its processing in Assessor.In;
...
> All this complexity may look like overkill, but this may be a good selling point when you are making project in collaboration with enterprise guys. They can “replace and reuse components” of the system and actual business logic is located only in one part of the system. We spend few man days discussing it.
...
Key responsibilities:
- Provide a REST JSON API for the investor-facing front-end.
- Persist portfolio criteria in the database.
- Place buy offers according to that criteria.
- Guard portfolio structure (no surprise drift).
...
<div className="grid gap-6 md:grid-cols-2 my-6">
  <div className="bg-card border border-border rounded-lg p-6 space-y-4">
    <h4 className="text-sm font-mono text-muted-foreground">Initial buy offers (10% per bucket)</h4>
    {initialBucketPlan.map((bucket) => (
      <BucketBar key={bucket.label} {...bucket} />
    ))}
  </div>
  <div className="bg-card border border-border rounded-lg p-6 space-y-4">
    <h4 className="text-sm font-mono text-muted-foreground">Rebalanced cycle (grow remaining demand)</h4>
    {rebalancedBucketPlan.map((bucket) => (
      <BucketBar key={bucket.label} {...bucket} />
    ))}
  </div>
</div>

After a bucket is filled we bump the others proportionally instead of letting high-risk demand starve the portfolio.
...
- Calculate sell/buy prices per investor configuration.
- Emit matched investments to the Asset Management System.
...
Each time Sell Offer comes we take its description and match versus all Buy Offers. On each match, we calculate sell and buy price (delta is our fee) and place a bid in a Sell Offer. When its volume is filled by 100% or partial sale is allowed — a message to Investments queue is published with Sell Offer and information about all the bids.
...
Key responsibilities:
- Account investor balances and investments.
- Comply with financial regulations.
- Notify the Trader about balance changes.
...
<SequenceDiagram className="my-6">
  <Participants>
    <Participant name="Matcher" position="left" color="primary" />
    <Participant name="Asset Management System" position="right" color="secondary" />
  </Participants>
  <Messages>
    <Message label="Batch matched investments" />
    <Message label="Debit investor balances" />
    <Message label="Publish balance deltas (30s window)" type="async" />
    <Message label="Notify Trader & CRM" type="response" direction="left" />
  </Messages>
  <Note text="We delay balance notifications slightly so bursts of trades result in a single update." />
</SequenceDiagram>
...
```text
Client → API Gateway → AuthN/AuthZ → Request Validation → Logging → Upstream Service
```

Instead of re-implementing cross-cutting concerns in every service we centralised them in an Elixir API gateway ([annon.api](http://github.com/nebo15/annon.api)).
...
After receiving the message we started a new supervised GenServer process that was responsible for applying business logi c. This proce ss was holding acknowledgment tag along with the data. And when, by any reason, supervisor terminated its child’s — tags were irreversibly lost.
